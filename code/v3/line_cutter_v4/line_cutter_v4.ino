...
const int timeOutCycles = 1000;
 /* 
 * Notes:
 * Make supercap voltage check non-blocking
 * Make a new spi2 object with correct clock pin
 * Add supercap voltage value to data packet
 * Add command to begin charging supercap
 */
// Edit spi library used by flash to switch clock pin
// Adjust for new cutting method (supercap)
 /*
...
bool chargeSupercap() {/*
  ...
  //if voltage is above what is reasonable after discharge, throw an error (.1V chosen arbitrarily)
  if (vcap > .1) {
    Serial.println("Supercap must be discharged before recharging");
    Serial.println("Current supercap voltage: ");
    Serial.print(vcap);
    return false;
  }

  //enable cap charging and delay 1 millisecond to reach steady state
  analogWrite(CAPC_EN, HIGH);
  delay(1);
  analogWrite(CHARGE_EN, HIGH);

  //while supercap voltage is below desired value, continue charging (7V chosen arbitrarily)
  if (vcap < 7) {
     vcapAnalog = 2 * analogRead(SNS_VCAP);
     vcap =  3.6 * (vcapAnalog / 1023.0);
  }
  
  //stop charging the supercap and return true
  analogWrite(CHARGE_EN, LOW);
  delay(1);
  analogWrite(CAPC_EN, LOW);
  
  Serial.println("Supercap is fully charged");
  Serial.println("Current supercap voltage: ");
  Serial.print(vcap);*/
  return true;
}
