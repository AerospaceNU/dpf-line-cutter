...
#define ACCEL_OUT_SIZE 14  // No longer sizeof since imu_out_t holds the output stuff, not just raw int16s
...
public:
  ...
  imu_out_t readout() {
    ...
    return {
      ...
      (int16_t) (((float) combine(buff[6], buff[7]) / 326.8) + 25.0),  // Why is not a float in the struct?
