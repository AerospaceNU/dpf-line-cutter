...
const int PHOTO_PIN = A6; //SNS_LIGHT
const int VOLTAGE_DIVIDER = A4; //SNS_BAT
const int SNS_VCAP = A7;
const int NICHROME_PIN1 = A0; //Gate control 1
const int NICHROME_PIN2 = 11; //Gate control 2
const int CHARGE_EN = 42; //No pin mapping in adafruit core (P1.06)
const int CHIP_SELECT_PIN = 8; //Flash spi CS pin
const int CHARGE_FI = 26; //standard spi sck
const int CAPC_EN = A1;
const int IMU_INT = 9; //might not be necessary since this int was left hanging in previous design

const int checkPeriod = 500;
const int timeOutCycles = 1000;
 /*
...
void setup() {
  //Disable supercap charging
  pinMode(CAPC_EN, OUTPUT);
  digitalWrite(CAPC_EN, LOW);
  pinMode(SNS_VCAP, INPUT);
  ...
  if (InternalFS.begin()) {
    ...
    Serial.println("Enter any keys to initialize internal file system"
    " (This formatting will destroy all of the data in External Flash!)");
  ...
  if (file) {
    ...
    if ( file.open(ID_FILE, FILE_O_WRITE) ) {
  ...
  // Set up flash
  SPI12.begin();
  SPI12.setBitOrder(MSBFIRST);
  SPI12.setClockDivider(SPI_CLOCK_DIV8);  // Divide the clock by 8
  SPI12.setDataMode(SPI_MODE0);
...
digitalWrite(NICHROME_PIN1, LOW);
  ...
  /*isCharged();
  int vcapAnalog = 2 * analogRead(SNS_VCAP);
  double vcap =  3.6 * (vcapAnalog / 1023.0);
  Serial.println(vcap);*/
}
...
void parse_command() {
  ...
  else if (command.substring(0, 6).equals("charge")) {
    startCharging();
  }
  ...
  else if (command.substring(0, 5).equals("test1")) {
    hardwarePWM1.writePin(NICHROME_PIN1, 0);
  }
  else if (command.substring(0, 5).equals("test2")) {
    hardwarePWM1.writePin(NICHROME_PIN1, 255);
  }
  else if (command.substring(0, 5).equals("test3")) {
    hardwarePWM2.writePin(NICHROME_PIN2, 0);
  }
  else if (command.substring(0, 5).equals("test4")) {
    hardwarePWM2.writePin(NICHROME_PIN2, 255);
  }
...
void readFlightVariables() {
  if (file.open(FLIGHT_VARIABLE_FILE, FILE_O_READ)) {
    ...
    Serial.println("Enter any keys to initialize flight variables");
    while ( !Serial.available() ) delay(1);
    // Requirements for state transitions
    currentFlightVars.limitVel         = -0.5;  // meters/second
    currentFlightVars.altitude1        = 457;  // Disreefing altitudes, in meters (higher one first!!)
    currentFlightVars.altitude2        = 366;
    currentFlightVars.disreefDelay1    = 48000;  // Max delay after ejection before first disreef, in milliseconds
    currentFlightVars.disreefDelay2    = 12500;  // Max delay after first disreef before second disreef, in milliseconds
    // PWM settings
    currentFlightVars.pwmVoltage1      = 1.7;  // Voltage applied to nichrome for line cuts
    currentFlightVars.pwmVoltage2      = 1.7;
    currentFlightVars.pwmDuration      = 3000;  // Length of PWM, in milliseconds
    // Photoresistor variables
    currentFlightVars.lightThreshold   = 400;  // Anything above this value is considered to be outside of tube
    currentFlightVars.lightTriggerTime = 2000;  // Continuous time that it must be light for board to detect deployment, in milliseconds
    InternalFS.remove(FLIGHT_VARIABLE_FILE);
    if ( file.open(FLIGHT_VARIABLE_FILE, FILE_O_WRITE) ) {
    uint8_t* contents = ( uint8_t* ) &currentFlightVars;
    ...
    if ( file.open(FLIGHT_VARIABLE_FILE, FILE_O_READ) ) {
      ...
      Serial.println(currentFlightVars.limitVel);
      Serial.println(currentFlightVars.altitude1);
      Serial.println(currentFlightVars.altitude2);
      Serial.println(currentFlightVars.disreefDelay1);
      Serial.println(currentFlightVars.disreefDelay2);
      Serial.println(currentFlightVars.pwmVoltage1);
      Serial.println(currentFlightVars.pwmVoltage2);
      Serial.println(currentFlightVars.pwmDuration);
      Serial.println(currentFlightVars.lightThreshold);
      Serial.println(currentFlightVars.lightTriggerTime);
    } else {
...
int altPWMLevel(double targetVoltage) {
  return (targetVoltage / 3.8) * 255;
}
...
void updateDataStruct() {
  ...
  currentData.supercapSense = analogRead(SNS_VCAP);
...
void sendSensorData() {
  ...
  bleuart.printf("Supercap [%f]\n", currentData.supercapSense * 2.0 * 3.6 / 1023.0);
...
bool isCharged(){
//while supercap voltage is below desired value, continue charging
  int vcapAnalog = 2 * analogRead(SNS_VCAP);
  double vcap =  3.6 * (vcapAnalog / 1023.0);
  if (vcap > 3.5) {
    //stop charging the supercap and return true
     digitalWrite(CAPC_EN, LOW);
     return true;
  } else {
     return false;
...
bool flashErase() {
  ...
  bleuart.printf("Erasing...\n");
  while (!completed) {
    completed = flash.is_erase_complete();
    delay(10);
  }
  uint32_t endTime = millis();
  bleuart.printf("Erase took [%f ms]\n", endTime - startTime);
  return true;
}
