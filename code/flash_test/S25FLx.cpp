...
//A great little tool for printing a uint8_t as binary without it chopping off the leading zeros.
...
void Flash::write(unsigned long loc, uint8_t *array, unsigned long length)
{
  ...
  if (length <= 255)
  {
    ...
    {
      Serial.print("loc & 0xff = ");
      Serial.println(loc & 0xff);
