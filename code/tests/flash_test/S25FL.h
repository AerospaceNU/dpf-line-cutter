...
class S25FL
{
 public:
  ...
  bool read_start(uint32_t startLoc, uint32_t numBytes, uint8_t *pData)
  {
    ...
    SPI.transfer(startLoc >> 24);
    SPI.transfer(startLoc >> 16);
    SPI.transfer(startLoc >> 8);
  ...
  bool write_start(uint32_t startLoc, uint32_t numBytes, uint8_t *data)
  {
  ...
  bool check_connected()
  {
    ...
    SPI.transfer(0x9F);
    byte manufacturer_id = SPI.transfer(0);
    byte mem_type = SPI.transfer(0);
    byte density = SPI.transfer(0);
    byte reg4Dh = SPI.transfer(0);
    byte arch = SPI.transfer(0);
    byte family = SPI.transfer(0);
